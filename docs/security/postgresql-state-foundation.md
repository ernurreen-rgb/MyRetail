...
Актуальное продуктовое и архитектурное решение хранится в Notion. Этот файл описывает
foundation-контракт рядом с кодом; controlled production enablement описан в
`docs/security/postgresql-controlled-enablement.md`.
...
- Production SQLite запрещён. PostgreSQL startup разрешён только с явным controlled latch и
  ...
  operational evidence Phase 6B.2–6B.3.
...
`MYRETAIL_STATE_MIGRATION_DATABASE_URL`. Каждый job также явно задаёт
`MYRETAIL_ENVIRONMENT=development|test|production` и
`MYRETAIL_STATE_MIGRATION_SSL_MODE=disable|require|verify-ca|verify-full`; в production
разрешён только `verify-full`. URL, SQL parameters и credentials не выводятся в логи.
...
До открытия traffic тот же API artifact и application-role environment проверяются командой
`myretail-state-preflight`. Она выполняет полный startup contract, закрывает pool и не применяет
migrations.
...
Текущий package-owned head — `20260718_06`. Startup дополнительно проверяет отсутствие
