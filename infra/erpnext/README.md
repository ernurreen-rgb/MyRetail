...
Сценарий создаёт роль `MyRetail API Reader` с правом только на чтение `Item`, создаёт пользователя `myretail-api@local.test`, выпускает API-ключи и записывает их в игнорируемый файл `services/api/.env`. Повторный запуск обновляет ключи. Секреты не выводятся в терминал.
